...
def cmd_push():
    ...
    if _git(cwd, 'rev-parse', '--git-dir').returncode != 0:
        ...
        u = sys.argv[2] if len(sys.argv) > 2 and '://' in sys.argv[2] else ('' if skip else (input(f"Create '{os.path.basename(cwd)}' on GitHub? (y=public/p=private): ") if shutil.which('gh') else input("Remote URL: ")).strip())
