...
def run():
    ...
    def _sshd_port(): return 8022 if os.getenv('TERMUX_VERSION') else 22

    with db() as c:
        c.execute("CREATE TABLE IF NOT EXISTS ssh(name PRIMARY KEY,host,pw)"); hosts = list(c.execute("SELECT name,host FROM ssh")); hmap = {r[0]: r[1] for r in hosts}
    ...
    if wda == 'stop': sp.run(['pkill', '-x', 'sshd']) if os.getenv('TERMUX_VERSION') else sp.run(['sudo', 'pkill', '-x', 'sshd']); print("✓ sshd stopped" if not _sshd_running() else "x failed"); return
    if wda in ('status', 's'): u, ip, p = os.getenv('USER', 'u0_a'), _sshd_ip(), _sshd_port(); print(f"{'✓ RUNNING' if _sshd_running() else 'x STOPPED'}  ssh {u}@{ip} -p {p}"); return
    if not wda: u, ip, p = os.getenv('USER', 'u0_a'), _sshd_ip(), _sshd_port(); shutil.which('ssh') or print("! pkg install openssh"); print(f"SSH {'✓ ON' if _sshd_running() else 'x OFF'}  →  ssh {u}@{ip} -p {p}\n  start/stop/status  server control\n  setup              install & configure\n  self [name]        add this device\n  add/rm/mv <name>   manage hosts\nHosts:"); [print(f"  {i}. {n}: {h}{' [pw]' if _pw(n) else ''}") for i,(n,h) in enumerate(hosts)] or print("  (none)"); print("\nAI: aio run # 'cmd'"); return
    if wda == 'setup': ip = _sshd_ip(); u = os.getenv('USER', 'user'); ok = _sshd_running(); cmd = 'pkg install -y openssh && sshd' if os.getenv('TERMUX_VERSION') else 'sudo apt install -y openssh-server && sudo systemctl enable --now ssh'; (not ok and input("SSH not running. Install? (y/n): ").lower() in ['y', 'yes'] and sp.run(cmd, shell=True)); ok = ok or _sshd_running(); print(f"This: {os.getenv('HOSTNAME', 'host')} ({u}@{ip}:{_sshd_port()})\nSSH: {'✓ running' if ok else 'x not running'}\n\nTo connect here from another device:\n  ssh {u}@{ip} -p {_sshd_port()}"); return
    ...
    if wda == 'self': u,p = os.getenv('USER','user'), _sshd_port(); ip = sp.run("hostname -I 2>/dev/null | awk '{print $1}'", shell=True, capture_output=True, text=True).stdout.strip() or _sshd_ip(); h = f"{u}@{ip}" + (f":{p}" if p != 22 else ""); n = (sys.argv[3:]or[u])[0]; pw=input("Pw: ").strip(); sp.run(f'sshpass -p "{pw}" ssh -o StrictHostKeyChecking=no localhost exit',shell=True,capture_output=True).returncode and _die("x pw"); _pw(n,pw); (c:=db()).execute("INSERT OR REPLACE INTO ssh(name,host)VALUES(?,?)",(n,h)); c.commit(); emit_event("ssh","add",{"name":n,"host":h,"pw":pw}); db_sync(); print(f"✓ {n}={h}"); return
    if wda in ('info','i'): [print(f"{n}: ssh {'-p '+hp[1]+' ' if len(hp:=h.rsplit(':',1))>1 else ''}{hp[0]}") for n,h in hosts]; return
    if wda == 'rm' and len(sys.argv) > 3: a=sys.argv[3]; n=hosts[int(a)][0]if a.isdigit()else a; _pw(n) and kr and kr.delete_password('aio-ssh',n); (c:=db()).execute("DELETE FROM ssh WHERE name=?",(n,)); c.commit(); emit_event("ssh","archive",{"name":n}); db_sync(); print(f"✓ rm {n}"); return
    ...
    pw and not shutil.which('sshpass') and _die("x sshpass"); print(f"→ {nm}", file=sys.stderr); os.execvp('sshpass',['sshpass','-p',pw]+cmd) if pw else os.execvp('ssh',cmd)
