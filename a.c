    ...
    case $OS in
        ...
        debian)
            if [[ -n "$SUDO" ]]; then export DEBIAN_FRONTEND=noninteractive
                $SUDO apt update -qq && $SUDO apt install -yqq clang libclang-rt-dev tmux git curl python3-pip sshpass rclone rsync tcc gcc cppcheck adb gir1.2-webkit-6.0 2>/dev/null; $SUDO apt install -yqq frama-c-base 2>/dev/null || true
