...
function show(p){wm.classList.remove('x');for(var k in views)window[views[k]].classList.toggle('on',k===p);if(p==='/term'&&F){connect();setTimeout(function(){F.fit();T.focus()},0);}if(p==='/note')fetch('/note-list').then(r=>r.text()).then(h=>nl.innerHTML=h)}
...
omni.onsubmit=function(e){e.preventDefault();var t=performance.now(),raw=qi.value.trim(),v=raw;if(!v)return;qo.innerHTML='';if(views['/'+v]){go('/'+v);qi.value='';console.log('omni total '+(performance.now()-t).toFixed(2)+'ms');return}var t2=performance.now();fetch('/api/omni',{method:'POST',headers:{'Content-Type':'application/x-www-form-urlencoded'},body:'q='+encodeURIComponent(v)}).then(function(r){return r.text()}).then(function(h){qo.innerHTML=h;qi.value='';qi.focus();console.log('omni fetch '+(performance.now()-t2).toFixed(2)+'ms')})};
