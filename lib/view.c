...
static int cmd_gui(int argc,char**argv){AB;perf_disarm();
...
#else
        }else{pcmd("S=$(ls $XDG_RUNTIME_DIR/sway-ipc.*.sock 2>/dev/null|head -1);"   /* bare grim = all outputs; one dead MST head fails the whole grab (shot.sh model: -o first output) */
            "O=$(SWAYSOCK=${S:-$SWAYSOCK} swaymsg -t get_outputs 2>/dev/null|grep -m1 '\"name\"'|cut -d'\"' -f4);"
            "grim ${O:+-o $O} /tmp/v_full.png 2>/dev/null||scrot /tmp/v_full.png 2>/dev/null;"
            "cp /tmp/v_full.png /tmp/v.png 2>/dev/null;identify -format '%w %h' /tmp/v_full.png 2>/dev/null",o,256);
#endif
        }char vp[P];snprintf(vp,P,"%s/v.png",td);
        if(!fexists(vp))return puts("x shot: no capture (display? grim/scrot?)"),1;   /* a path printed for a file that isn't there reads as success */
        printf("%s\n%s\n",o,vp);return 0;}
    if(*s=='f'&&argc>=7){ /* fan */
    ...
    if(*s=='k'&&argc>=4){char k[512],c[B];ajoin(k,512,argc,argv,3); /* key */
    ...
    if(*s=='u'&&argc>=4){if(!vwsl()){puts("uia: WSL only");return 1;} /* uia */
        char nm[512],c[B],o[64];ajoin(nm,512,argc,argv,3);
    ...
    if(!strcmp(s,"js")&&argc>=4){char js[B],o[B];ajoin(js,B,argc,argv,3); /* js eval */
        vcdp(js,o,B);printf("%s\n",o);return 0;}
    if(!strcmp(s,"nav")&&argc>=4){char js[B],o[B];   /* was a 2nd copy of vcdp's websocket block in PowerShell; now shares vcdp (note: vcdp writes v_js.txt to the WSL path, so the linux CDP leg is still dead) */
        snprintf(js,B,"location.href='%s'",argv[3]);vcdp(js,o,B);printf("nav: %s\n",argv[3]);return 0;}
    ...
    if(!strcmp(s,"links")){char o[B]; /* clickable elements */
        ...
        printf("%s\n",o);return 0;}
    if(!strcmp(s,"find")&&argc>=4){char js[B],o[B],q[512];ajoin(q,512,argc,argv,3); /* find+click by text */
