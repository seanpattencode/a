...
if not os.path.exists(PRE+"/lib/libevent.a"):
    ...
    sh(f"./configure --host=aarch64-linux-android --prefix={PRE} --disable-shared --enable-static --disable-openssl --disable-samples --disable-libevent-regress --disable-debug-mode >/dev/null && make -j{J} >/dev/null && make install >/dev/null", f"{SRC}/libevent-2.1.12-stable")
...
if not os.path.exists(f"{NC}/progs/tic"):
    ...
    sh(f"./configure --host=aarch64-linux-android --prefix={PRE} --without-shared --without-ada --without-tests --without-manpages --without-debug --without-cxx-binding --with-default-terminfo-dir=/data/local/tmp/terminfo --disable-stripping >/dev/null && make -j{J} >/dev/null && (make install >/dev/null 2>&1 || true)", NC)
