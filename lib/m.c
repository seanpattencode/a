...
static int mm_stream(const char *sf, const char *sp, char *a, size_t sz, char *bash, size_t bsz) {
    ...
    if (!cp) {
        ...
        if (is_gemini) { char g[B];
            snprintf(g,B,"F='%s';awk 'NR==FNR{if(/^## a-archive [0-9]/){A[++n]=$3}else if(/^## a-archive-undo/){if(n)n--}next}{h=0;for(i=1;i<=n;i++){split(A[i],r,\"-\");if(FNR>=r[1]+0&&FNR<=r[2]+0){h=1;break}}if(/^## a-archive/||h)next;if(/^## a-loaded /){s=1;next}if(s&&/^## a-loaded-end$/){s=0;next}if(!s)print}' \"$F\" \"$F\"|gemini -p '' --skip-trust --approval-mode plan -o stream-json -m '%s'",sf,md);
            execlp("sh","sh","-c",g,(char*)0); }
        else if (is_codex) { char x[B],t[80]="";
            if (has_tier) snprintf(t,80," -c 'service_tier=\"%s\"'",tier);
            snprintf(x,B,"F='%s';awk 'NR==FNR{if(/^## a-archive [0-9]/){A[++n]=$3}else if(/^## a-archive-undo/){if(n)n--}next}{h=0;for(i=1;i<=n;i++){split(A[i],r,\"-\");if(FNR>=r[1]+0&&FNR<=r[2]+0){h=1;break}}if(/^## a-archive/||h)next;if(/^## a-loaded /){s=1;next}if(s&&/^## a-loaded-end$/){s=0;next}if(!s)print}' \"$F\" \"$F\"|iconv -f UTF-8 -t UTF-8 -c|codex exec --json --skip-git-repo-check --dangerously-bypass-approvals-and-sandbox --disable shell_tool --disable unified_exec --disable tool_search --disable tool_suggest -m '%s' -c 'model_reasoning_effort=\"%s\"'%s",sf,md,ef,t);
            execlp("sh","sh","-c",x,(char*)0);
        } else { char x[B*2];
            snprintf(x,B*2,"F='%4$s';awk 'NR==FNR{if(/^## a-archive [0-9]/){A[++n]=$3}else if(/^## a-archive-undo/){if(n)n--}next}{h=0;for(i=1;i<=n;i++){split(A[i],r,\"-\");if(FNR>=r[1]+0&&FNR<=r[2]+0){h=1;break}}if(/^## a-archive/||h)next;if(/^## a-loaded /){s=1;next}if(s&&/^## a-loaded-end$/){s=0;next}if(!s)print}' \"$F\" \"$F\"|claude -p --output-format stream-json --include-partial-messages --verbose --tools '' --model '%1$s' --effort '%2$s' --system-prompt \"$1\" --append-system-prompt-file <(cat %3$s/local/a_cat.txt 2>/dev/null; printf '\\n==> i.txt <==\\n'; cat %3$s/m/i.txt 2>/dev/null) --settings '{\"enabledPlugins\":{\"clangd-lsp@claude-plugins-official\":false}}'",md,ef,AROOT,sf);
    ...
    for (;;) {
        ...
        if (bk[i]=='T') {
            ...
            if(!WEXITSTATUS(r)&&(strstr(OPS[bi[i]].l,"archive")||!strcmp(OPS[bi[i]].l,"undo"))){
                char rc[B];snprintf(rc,B,"F=$(cat %s/m_file 2>/dev/null||echo m.txt);tmux respawn-pane -k -t :.0 \"tail -Fn +1 %s/m/$F | grep -v --line-buffered '^## a-archive'\"",DDIR,AROOT);
                (void)!system(rc);
            }
        }
    }
...
/* append-only ## a-archive L1-L2 marker (file never shrinks). conv pane filters via grep -v. */
...
static int m_archive(int c, char **v) {
    ...
    if (c > 3 && (!strcmp(v[3],"undo")||!strcmp(v[3],"restore")||!strcmp(v[3],"unarchive"))) {
        ...
        char xp[P]; snprintf(xp,P,"%s/m/m.txt",AROOT);
        FILE *f=fopen(xp,"a"); if(!f){perror("open");return 1;} fprintf(f,"## a-archive-undo\n"); fclose(f);
        char x[B]; snprintf(x,B,"git -C '%1$s/m' add -A && git -C '%1$s/m' commit -q -m archive-undo",AROOT);
        (void)!system(x); puts("undone"); return 0;
    }
...
static int m_reinit(const char *fn) {
    ...
    snprintf(c,B,"tmux respawn-pane -k -t :.0 \"tail -Fn +1 %s/m/%s | grep -v --line-buffered '^## a-archive'\"",AROOT,fn); (void)!system(c);
...
static int cmd_m(int c, char **v) {
    ...
    snprintf(b, B, "[ -d %1$s/m ]||(cd %1$s&&gh repo create m --private --clone);"
                   ...
                   "$S -b \"tail -Fn +1 %2$s | grep -v --line-buffered '^## a-archive'\";"
