...
static void m_run(const char*sf,const char*wd){ /* loop: model -> last CMD: -> run in wd -> feed back */
    ...
    for(int i=0;i<25&&!g_halt;i++){  
        ...
        if(tty){fputs("\033[2m⠋ 0.0s\033[0m",stdout);fflush(stdout);}  
        while(!g_halt){
        ...
        m_ap(sf,"assistant",b);
        char*t=0;for(char*s=b;(s=strstr(s,"CMD:"));s+=4)if(s==b||s[-1]=='\n')t=s;  /* LAST line-start match: prose CMD: mentions must never run */
        if(!t||g_halt)break;
        char*cm=t+4;while(*cm==' '||*cm=='`')cm++;
        char*e=strchr(cm,'\n');if(e)*e=0;
        for(e=cm+strlen(cm);e>cm&&(e[-1]=='`'||e[-1]==' ');)*--e=0;
        if(!strcmp(cm,last)){if(++rep>=3)break;}else rep=0;  /* 3 identical in a row = loop */
...
static size_t m_input(char **out,const char *sfn,int menu){
    ...
    for(;;){
        ...
        if(c==27){char s[8];int av=0;usleep(2000);ioctl(0,FIONREAD,&av);if(!av){l=0;break;}  
