...
static pid_t bg(void){pid_t c=fork();if(!c){setsid();int d=open("/dev/null",O_WRONLY);dup2(d,1);dup2(d,2);}return c;}   /* detached child, stdout/err nulled: never holds a caller's pipe open */
