  ...
  const dispatch = async (m) => {
    try {
      switch (m.action) {
        ...
        case 'open':     return {ok:true, value:await browser.runtime.sendMessage({action:'open',url:m.url})};
        ...
        case 'screenshot': return {ok:true, value:await browser.runtime.sendMessage({action:'screenshot',format:m.format})};
        default: return {error:'unknown action: '+m.action};
      }
    } catch (e) { return {error:String(e)}; }
  };
  // CSP-locked sites (gemini, chatgpt, claude.ai) block content-script fetch in
  // Firefox; fall back to background.runtime.sendMessage which is CSP-exempt.
  const xfer = async (url, opts) => {
    try { return await fetch(url, opts); }
    catch { return browser.runtime.sendMessage({a:'fetch', url, opts}); }
  };
  const post = d => xfer(RESP, {method:'POST', headers:{'Content-Type':'application/json'}, body:JSON.stringify({src:location.href, ...d})});
  post({hello:location.href, title:document.title, source:'extension', v:'0.8'});
  let chain = Promise.resolve();
  const loop = async () => {
    let r;
    try { r = await xfer(POLL); } catch (e) { setTimeout(loop, 2000); return; }
    loop();  // re-register the poll IMMEDIATELY — was only after dispatch, so commands arriving
             // during a (possibly multi-second) dispatch were dropped. Dispatch now runs on a
             // serialized side-chain (in-order, but never blocks the next poll).
    const c = r && r.status === 200 ? (r.json ? await r.json() : r.body) : null;
    if (c) chain = chain.then(async () => {
      try { const out = await dispatch(c); await post({id:c.id, ...out}); }
      catch (e) { post({id:c.id, error:String(e)}); }
    });
  };
  loop();
})();
