...
(() => {
  ...
  post({hello: location.href, title: document.title, source:'userscript-bri', v:'0.8'});
  let chain = Promise.resolve();
  const loop = async () => {
    let r;
    try { r = await bridge_fetch(POLL); } catch(e) { setTimeout(loop, 2000); return; }
    loop();  // re-register the poll IMMEDIATELY — don't wait for dispatch (that gap dropped commands)
...
// open+focus a tab — deduped by URL so the bridge's broadcast-to-all-tabs
// opens ONE tab (or focuses an existing one), not N. has/set runs synchronously
// before any await, so concurrent messages share the one create-promise.
  ...
  if (msg.action === 'open') {
    ...
    if (!_opening.has(key)) _opening.set(key, (async () => {
      ...
      const tab = hit ? await browser.tabs.update(hit.id, {active:true})
                      : await browser.tabs.create({url: msg.url, active:true});
