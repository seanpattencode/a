...
static void fallback_py(const char *mod, int argc, char **argv) {
    ...
    if(!fexists(path))strcpy(path+strlen(path)-2,"c");   /* polyglot module (py half in #if 0) */
...
static int create_sess(const char *sn, const char *wd, const char *cmd, const char *extra) {
    ...
    char sid[64]="",acmd[B];
    if(cmd&&(strstr(cmd,"claude ")||strstr(cmd,"grok "))&&!strstr(cmd,"--resume")&&!strstr(cmd,"--continue"))   
