...
set -eu
L=0; V=""; P=""; O=""
while getopts lv:p:o: o; do case $o in
...
a say [-l] [-v voice] [-p semitones] [-o out.wav] text…
  ...
  -p semitones, negative = lower pitch; -o file = write wav, don't play
...
[ "$N" -eq 0 ] && exit 0
if [ -n "$O" ]; then i=1  # -o file.wav: write instead of play
    while [ $i -le $N ]; do gen $i "$(cat $D/t_$i)"; echo "file '$D/c_$i.wav'" >>$D/l; i=$((i+1)); done
    ffmpeg -y -loglevel error -f concat -safe 0 -i $D/l "$O"; exit 0
fi
gen 1 "$(cat $D/t_1)"
i=2
while [ $i -le $N ]; do
