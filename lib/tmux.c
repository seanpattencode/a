...
static void tm_ensure_conf(void) {
    ...
    fputs("# aio-managed-config\nset-hook -gu after-new-window\nset-hook -gu session-created\n"
        ...
        "set -g status-format[1] \"#[align=left,fg=colour232,bg=colour231,bold] #I:#W #[bg=default fg=white nobold] #{?#{e|>:#{session_windows},1},#[fg=white bold]#[range=user|prev]  <  #[norange] #[range=user|next]  >  #[norange] ,}#{W:#{?window_active,,#[range=window|#{window_index}]#{?window_bell_flag,#[fg=white bg=red bold],#[fg=colour231 bg=colour243]} #{?window_bell_flag,\\U0001F534 ,}#I:#W #[default]#[norange] }}\"\n"
