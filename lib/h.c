...
static const hm M_sys[]={{'f',"fleet","devices: sync/agents/ram","fleet",0},{'l',"list projects/cmds","show all entries","ls",0},{'k',"kill sessions","stop all agents","kill",0},{'u',"update","pull and rebuild","update",0},{'g',"config","edit settings","config",0},{'p',"perf","speed limits","perf",0},{'c',"calendar","view schedule","cal",0},{0}};
static const hm M_top[]={{'/',"claude code","type → launch Claude Code",0,0},{'P',"prompt","edit/switch prompt files","prompt",0},{'a',"Agents","start AI agents",0,M_ag},{'f',"Feed","review agents: park/resume","feed",0},{'g',"Git","sync and push",0,M_git},{'p',"Projects","open by number","ls",0},{'n',"Notes","capture ideas","note",0},{'t',"Tasks","prioritized todo list","task",0},{'h',"Hub / schedule","scheduled operations","hub",0},{'c',"Code dump","source to clipboard","cat",0},{'s',"ssh","connect to device","ssh",0},{'y',"System","status and config",0,M_sys},{0}};
static int cmd_h(int c,char**v){(void)c;(void)v;perf_disarm();
    ...
    struct timespec tk=_t0; const char*act="render"; char actb[48];  /* tk: per-frame timer; act: what produced this frame, so the footer says what it measured (e.g. →Agents on a submenu transition) */
    for(;;){const hm*m=stk[sp];char b[B];int l=0;
        ...
        l+=snprintf(b+l,(size_t)(B-l),"\n\033[90m  key=select · esc/q=%s · \033[37m%s %.3fms\033[0m\n",sp?"back":"quit",act,fms);
        ...
        if(ch==27||ch=='q'||ch==3){if(sp){sp--;act="←back";continue;}break;}
        ...
        for(int i=0;m[i].l;i++)if(m[i].k==ch){
            if(m[i].s){if(sp<15){ttl[sp+1]=m[i].l;stk[++sp]=m[i].s;snprintf(actb,sizeof actb,"→%s",m[i].l);act=actb;}}
