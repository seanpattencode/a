...
static const hm M_top[]={{'a',"Agents","start AI agents",0,M_ag},{'g',"Git","sync and push",0,M_git},{'p',"Projects","open by number","ls",0},{'n',"Notes","capture ideas","note",0},{'t',"Tasks","prioritized todo list","task",0},{'h',"Hub / schedule","scheduled operations","hub",0},{'c',"Code dump","source to clipboard","cat",0},{'s',"ssh","connect to device","ssh",0},{'y',"System","status and config",0,M_sys},{0}};
static int cmd_h(int c,char**v){(void)c;(void)v;perf_disarm();
    ...
    for(;;){const hm*m=stk[sp];char b[B];int l=0;
        ...
        l+=snprintf(b+l,(size_t)(B-l),"\n\033[90m  key=select · esc/q=%s\033[0m\n",sp?"back":"quit");
