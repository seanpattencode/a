# a tokreal [dir]: repo tokens as Codex (tiktoken o200k_base, free) and Claude = Fable 5.1 (claude -p usage minus baseline; Haiku 4.5 counts 25% lower; asks first)
...
def n(x):
 open('/tmp/tr','w').write(x);r=S.run(['claude','-p','--model','fable','--output-format','json','ok']+['--append-system-prompt-file','/tmp/tr']*(x>''),stdin=S.DEVNULL,**K).stdout
