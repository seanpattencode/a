...
/* cal data layer — defined in lib/cal.c (included after note.c); flow's CAL page calls into it */
typedef struct{char ln[200];int fi;}CEv;
static char CFp[64][P];
static int cal_load(const char*,CEv*,int);static char*cal_write(const char*,const char*);
static int cal_arch(const char*,CEv*);static void cal_disp(const char*,char*,size_t);
...
static void hl_kw(const char*s){for(const char*p=s;*p;){const char*e;  /* [bracketed] keywords → cyan */
    if(*p=='['&&(e=strchr(p,']'))){printf("\033[36m%.*s\033[0m",(int)(e-p+1),p);p=e+1;}else putchar(*p++);}}
static int cmd_task(int argc,char**argv){
    ...
    if(!sub||!strcmp(sub,"top")||!strcmp(sub,"new")){int n=load_tasks(dir);
        ...
        for(int i=0;i<k;i++){char fr[24];task_front(T[i].d,fr,24);printf("  %2d %s \033[90mP%s\033[0m ",i+1,fr,T[i].p);hl_kw(T[i].t);putchar('\n');}
        ...
        if(ch=='n'){printf("new task> ");fflush(stdout);char lb[B];  /* new-task button: type → add (+rapid continue) */
            if(fgets(lb,B,stdin)&&lb[0]!='\n'){lb[strcspn(lb,"\n")]=0;execvp("a",(char*[]){"a","t","add",lb,NULL});}
            execvp("a",(char*[]){"a","t",NULL});}
...
/* archive a flow item by its display id (n1..n4 notes, t1..t4 tasks, p1..p4 prompts, s1..s4 suggestions); re-applies the active sort so the id matches the screen */
static int flow_archive(const char*id,const char*srt){
    ...
    else{snprintf(dd,P,"%s/%s",SROOT,id[0]=='n'?"notes":id[0]=='s'?"suggestions":"prompts");int m=load_notes(dd,NULL);qsort(gn,(size_t)m,sizeof(GN),gncmp);
...
static int cmd_flow(int argc,char**argv){perf_disarm();
    ...
    const char*srt="pri";int verbose=0,pg=(tty&&isatty(0))?0:-1;   /* a flow [pri|new|due] [v] [all|0-5] */
    for(int i=2;i<argc;i++){if(!strcmp(argv[i],"v")||!strcmp(argv[i],"verbose"))verbose=1;else if(!strcmp(argv[i],"all"))pg=-1;
        else if(isdigit((unsigned char)argv[i][0])&&!argv[i][1])pg=(argv[i][0]-'0')%6;else srt=argv[i];}
    ...
    static CEv cev[256];   /* CAL page state — filled by PG(5) render, read by [e]/[c] */
    for(;;){
    int bynew=!strcmp(srt,"new")||!strcmp(srt,"date"),bydue=!strcmp(srt,"due");
    int lim=4,cnt=-1,ncv=0,civ=0;char dir[P],top[P],topt[2048];*top=0;*topt=0;   /* all-mode shows 4 each; cnt + top/topt (current item's file/text) feed the strip + [o]/[m]/[s] */
    ...
    /* SUGGESTIONS — machine-generated proposals (lib/suggest.sh), quarantined from the human queues:
       nothing enters notes/tasks/prompts until [m] promotes it (routed by its TASK:/PROMPT:/NOTE: tag).
       [e] dismisses. The m keypress IS the gate between machine output and the human-authored corpus. */
    if(PG(3)){snprintf(dir,P,"%s/suggestions",SROOT);int ns=load_notes(dir,NULL);cnt=ns;qsort(gn,(size_t)ns,sizeof(GN),gncmp);
    if(pg>=0){if(ns){int ix=ns-1;const char*u=strrchr(gn[ix].p,'_');char fr[24],ts[16]="",b2[B];if(u)snprintf(ts,16,"%.15s",u+1);ts_date(u?ts:NULL,fr,24);
        snprintf(b2,B,"  %ss1 %-14s%s %s",DM,fr,X,gn[ix].t);snprintf(top,P,"%s",gn[ix].p);snprintf(topt,2048,"%s",gn[ix].t);place1(b2);}}
    else{printf("\n%s━━ SUGGESTIONS (%d)%s %smachine proposals — m promotes by tag, e dismisses%s\n",BO,ns,X,DM,X);
    for(int i=0;i<ns&&i<lim;i++){int ix=ns-1-i;const char*u=strrchr(gn[ix].p,'_');char fr[24],ts[16]="";if(u)snprintf(ts,16,"%.15s",u+1);ts_date(u?ts:NULL,fr,24);
        printf("  %ss%d %-14s%s %s\n",DM,i+1,fr,X,gn[ix].t);}
    if(ns>lim)printf("  %s…+%d more%s\n",DM,ns-lim,X);
    if(!ns)printf("  %s(none — lib/suggest.sh generates)%s\n",DM,X);}}
    ...
    /* CAL — TRIAL (Sean 2026-06-12): personal schedule surfaced in the home loop. Rationale: some
       tasks route to ME not an llm, and some llm convos end with me `a cal`-ing something to do.
       REMOVE this page from a flow if over time it makes no sense / goes unused — `a cal` stays
       the full standalone TUI either way. cal/ is never injected into agent prompts (a j a). */
    if(PG(5)){snprintf(dir,P,"%s/cal",SROOT);ncv=cal_load(dir,cev,256);cnt=ncv;
    char tdy[16];{time_t nw2=time(NULL);strftime(tdy,16,"%Y-%m-%d",localtime(&nw2));}
    civ=ncv?ncv-1:0;for(int i=0;i<ncv;i++)if(strncmp(cev[i].ln,tdy,10)>=0){civ=i;break;}   /* c1 = next upcoming (last past if none — e dismisses stale) */
    if(pg>=0){if(ncv){char dp[224],b2[B];cal_disp(cev[civ].ln,dp,224);
        snprintf(b2,B,"  %sc1%s %s",DM,X,dp);snprintf(top,P,"%s",CFp[cev[civ].fi]);snprintf(topt,2048,"%s",cev[civ].ln);place1(b2);}}
    else{printf("\n%s━━ CAL (%d)%s %supcoming — a cal = full TUI%s\n",BO,ncv,X,DM,X);
    for(int i=civ;i<ncv&&i<civ+lim;i++){char dp[224];cal_disp(cev[i].ln,dp,224);printf("  %sc%d%s %s\n",DM,i-civ+1,X,dp);}
    if(ncv>civ+lim)printf("  %s…+%d more%s\n",DM,ncv-(civ+lim),X);
    if(!ncv)printf("  %s(none — a cal add [6-14] [15:00] text)%s\n",DM,X);}}
    ...
    static const char*CT[]={"note","task","prompt","suggestion","note","event"};const char*ct=CT[pg<0?0:pg];char tc=pg==1?'t':pg==2?'p':pg==3?'s':'n';
    ...
    if(ch=='j'){pg=pg<0?0:(pg+1)%6;continue;}
    if(ch=='k'){pg=pg<0?5:(pg+5)%6;continue;}
    ...
    if(ch=='m'){if(pg==0&&*topt){char d2[P];snprintf(d2,P,"%s/tasks",SROOT);mkdirp(d2);char*np=task_add(d2,topt,50000);   /* [m]ove↑ the ladder: note→task, task→prompt — source archived, exit receipt covers it */
        ...
        else if(pg==3&&*topt){const char*sg=topt;char d2[P],*np;const char*vb;   /* suggestion → queue routed by its tag: this keypress is the only door from machine output into the human queues */
            if(!strncmp(sg,"PROMPT:",7)){sg+=7;while(*sg==' ')sg++;snprintf(d2,P,"%s/prompts",SROOT);mkdirp(d2);np=note_save(d2,sg);vb="suggestion→prompt";}
            else if(!strncmp(sg,"NOTE:",5)){sg+=5;while(*sg==' ')sg++;snprintf(d2,P,"%s/notes",SROOT);mkdirp(d2);np=note_save(d2,sg);vb="suggestion→note";}
            else{if(!strncmp(sg,"TASK:",5)){sg+=5;while(*sg==' ')sg++;}snprintf(d2,P,"%s/tasks",SROOT);mkdirp(d2);np=task_add(d2,sg,50000);vb="suggestion→task";}
            do_archive(top);if(na<64)snprintf(af[na],P,"%s",fa_last);na++;edit_st(np,st,vb);ne++;}
        else snprintf(st,256,"m: note→task / task→prompt / suggestion→tag only (prompt: [s]end runs it)");continue;}
    if(ch=='e'){if(pg==5){if(ncv&&cal_arch(dir,&cev[civ])){snprintf(st,128,"✓ archived c1 — syncs on exit");ne++;}   /* cal = line surgery, not file rename — outside af[]/flow_exit; exit sync commits */
            else snprintf(st,128,"nothing to archive");}
        else if(pg>=0&&pg<=3){char id[3]={tc,'1',0};   /* gmail e: top of current page goes, next promotes into view */
    ...
    int acted=0;
    if(ch=='x'){char ib[16];printf("  archive (n1 t2 p3 s4 …)> ");fflush(stdout);   /* typed id = power path; only one item is on screen */
        if(fgets(ib,16,stdin)&&(ib[0]=='n'||ib[0]=='t'||ib[0]=='p'||ib[0]=='s')){ib[strcspn(ib,"\n")]=0;
    ...
    if(ch=='c'){char buf[B];   /* gmail c: compose — new item of the current page's type (note elsewhere); instant return, new item renders as top */
        ...
        if(fgets(buf,B,stdin)&&buf[0]!='\n'){buf[strcspn(buf,"\n")]=0;char d2[P],*path;
            ...
            else if(pg==5){snprintf(d2,P,"%s/cal",SROOT);path=cal_write(d2,buf);
                if(!path){snprintf(st,256,"x empty");continue;}}
            else{snprintf(d2,P,"%s/%s",SROOT,pg==2?"prompts":pg==3?"suggestions":"notes");mkdirp(d2);path=note_save(d2,buf);}
