...
+''.join(row(j+1,ls[hi[j]],'\n'.join(ls[hi[j]+1:hi[j+1]]).strip())for j in O)
 +f'<div class=top><span class=grp><button class=bb id=sR onpointerdown="tsort()">order: rank (s)</button><a class=bb href=/review style="text-decoration:none" title="spawned agents finish there — a done queue">review</a></span>'
 ...
 +'<span class=grp id=dgs><button class=bb onpointerdown="sp()" style="font-size:17px;font-weight:700;padding:8px 16px;background:#fff;color:#000" title="one agent per task, top N of the current order, each in its own tmux window">&#9654; spawn agents</button><input id=sn class=bb value=3 inputmode=numeric style="width:3.5em"><label class=d>on the top N tasks &#8594; results in review</label><select class=bb onchange="ga(this.value)">'+''.join('<option'+(' selected'if AG==x else'')+'>'+x+'</option>'for x in MO)+'</select>'+sel('m_model',MD or mo[0][0],mo[0])+(sel('m_effort',EF or mo[1][0],mo[1])if mo[1]else'')+sel('m_perms',PM,['bypass','ask'])+'</span><span class=grp><button class=bb onpointerdown="pv()">prompt · view/edit (o)</button></span></div>'
