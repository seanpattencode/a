...
def page(by=''): # one-at-a-time is client-side (rows stay in the DOM, prev/next toggle: sub-ms, no navigation) # /tasks[?by=date] — the file rendered; rank view = file order (instant local moves); date view = dated first, soonest first (actions reload)
 ...
 def row(i,h,body):
  ...
  return(f'<div class=tk data-i={i} style="margin:6px 0;border:1px solid {c};border-radius:8px;padding:5px 8px"><div style="font-size:15px"><span class=n>rank {i}</span> {dp}{e(re.sub(DT,"== ",re.sub(TAG,"",h)).strip("= ").strip())}</div>'
   +(f'<div style="color:#999;font-size:12.5px;white-space:pre-wrap;margin:2px 0 0">{e(pv)}</div>' if pv else'')
   +f'<div class=bt>{s}<button class=bb data-k=o onpointerdown="ed(this)">context · edit ({nl} lines) (o)</button>{B("done","done")} {B("down","rank down 10")} {B("rank","set rank")} <label class=bb style="font-size:12px">date <input type=datetime-local step=1 value="{dv}" onchange="tq(\'date\',this,this.value)" style="font:inherit;background:#000;color:#fff;border:0"></label>{B("date","clear date")if d else""} {B("agent","tag existing agent")}</div>'
...
+''.join(row(j+1,ls[hi[j]],'\n'.join(ls[hi[j]+1:hi[j+1]]).strip())for j in O)
 ...
 '<span class=grp><span class=d style="font-size:12px">view</span><button class=bb id=vL onpointerdown="view(0)">whole list (v)</button><button class=bb id=vO onpointerdown="view(1)">one at a time (v)</button></span>'
 ...
 'keys — j/k next/prev · e done · o context · i details · d down10 · r set rank · a agent · g spawn/resume/go · s order · v view · c new; act on the shown task (one-view) else rank 1<br>'
 ...
 'tr0.textContent=c+" · op "+op.toFixed(3)+"ms · painted "+pt.toFixed(2)+"ms · "+t.trim()+" · round trip "+(performance.now()-t0).toFixed(1)+"ms";if(t[0]=="x"||!local)setTimeout(()=>location.reload(),600)})}))}document.addEventListener("keydown",function(ev){if(ev.ctrlKey||ev.metaKey||ev.altKey)return;if(/INPUT|TEXTAREA|SELECT/.test((document.activeElement||{}).tagName||""))return;var q=ev.key;if(q=="j")return step(1);if(q=="k")return step(-1);if(q=="v")return view(document.body.classList.contains("one")?0:1);if(q=="s"){location=BYD?"/tasks":"/tasks?by=date";return}if(q=="c"){ev.preventDefault();tn.focus();return}var R=rows(),r=document.body.classList.contains("one")?R[K]:R[0],b=r&&r.querySelector("[data-k="+q+"]");if(b)b.dispatchEvent(new PointerEvent("pointerdown",{bubbles:true}))});</script>')
