...
+''.join(row(j+1,ls[hi[j]],'\n'.join(ls[hi[j]+1:hi[j+1]]).strip())for j in O)
 ...
 'var u="/tasks/run",bd="c="+encodeURIComponent(c);if(/^(spawn|resume)$/.test(v)){u="/tasks/"+v+"?n="+i;bd="";b.disabled=1;b.textContent="\u23f3 "+v+"ing task "+i+"\u2026";tar.textContent=b.textContent}if(v=="go"){u="/problems/go?w="+encodeURIComponent(x);bd="";local=1}'
