...
+''.join(row(j+1,ls[hi[j]],'\n'.join(ls[hi[j]+1:hi[j+1]]).strip())for j in O)
 ...
 'replaces the old a task · file: ~/a/adata/git/tasks.txt (a-git; local commits, fleet-synced)</div></details><div id=tr0 class=d>&nbsp;</div><div id=tar class=d style="min-height:18px"></div>'
