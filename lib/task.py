...
def page(by=''): # one-at-a-time is client-side (rows stay in the DOM, prev/next toggle: sub-ms, no navigation) # /tasks[?by=date] — the file rendered; rank view = file order (instant local moves); date view = dated first, soonest first (actions reload)
 ...
 def row(i,h,body):
  ...
  nl=body.count('\n')+1 if body else 0
  return(f'<div class=tk data-i={i} style="margin:12px 0;border:1px solid {c};border-radius:8px;padding:8px"><div style="font-size:21px"><span class=n>rank {i}</span> {dp}{e(re.sub(DT,"== ",re.sub(TAG,"",h)).strip("= ").strip())}</div>'
   f'<div class=bt>{s}<button class=bb onpointerdown="ed(this)">context · edit ({nl} lines)</button>{B("done","done")} {B("down","rank down 10")} {B("rank","set rank")} <label class=bb style="font-size:15px">date <input type=datetime-local step=1 value="{dv}" onchange="tq(\'date\',this,this.value)" style="font:inherit;background:#000;color:#fff;border:0"></label>{B("date","clear date")if d else""} {B("agent","tag existing agent")}</div>'
   f'<div class=nf hidden style="font-size:15px;margin-top:6px;border-top:1px solid #444;padding-top:6px">{dt}</div>'
 ...
 return('<style>.bt{display:flex;flex-wrap:wrap;gap:6px;align-items:center;font-size:15px;margin-top:5px}.bt>*{flex:none}.bt .bb{font-size:15px;text-decoration:none}@media(max-width:700px){.bt{flex-direction:column;align-items:stretch}}.n{background:#333;color:#ccc;border-radius:6px;padding:0 9px;font-size:14px;vertical-align:middle}.top{display:flex;flex-wrap:wrap;gap:10px;align-items:center;margin:0 0 10px}.grp{display:flex;gap:6px;align-items:center;border:1px solid #555;border-radius:9px;padding:5px 9px;background:#111}#cnt{min-width:6.5em;text-align:center;font:15px system-ui;color:#ccc}body.one .tk{display:none}body.one .tk.on{display:block}.nf div{display:flex;gap:9px;align-items:baseline;margin:4px 0}.nf b{background:#222;border:1px solid #555;border-radius:5px;padding:0 7px;min-width:7em;font:13px system-ui;color:#bbb;text-align:center}</style>' # thin: one button per line
 f'<div class=top><span class=grp><span class=d>order</span><a class=bb href=/tasks style="{"background:#245"if by!="date"else""}">by rank</a><a class=bb href="/tasks?by=date" style="{"background:#245"if by=="date"else""}">by date</a></span>'
 '<span class=grp><span class=d>view</span><button class=bb id=vL onpointerdown="view(0)">whole list</button><button class=bb id=vO onpointerdown="view(1)">one at a time</button></span>'
