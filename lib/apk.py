...
@JavascriptInterface fun native(name:String){h.post{nav?.invoke(name)}}   // web home menu -> a native view (setup/native/read/rec); localhost menu lists these when window.A exists
...
if(ms==null){ms=MediaSession(this,"a").apply{setCallback(object:MediaSession.Callback(){
override fun onPlay(){pjs("play")};override fun onPause(){pjs("pause")}})}
if(Build.VERSION.SDK_INT>=33&&checkSelfPermission(android.Manifest.permission.POST_NOTIFICATIONS)!=android.content.pm.PackageManager.PERMISSION_GRANTED)requestPermissions(arrayOf(android.Manifest.permission.POST_NOTIFICATIONS),9)}
val pl=st==1
ms!!.setMetadata(android.media.MediaMetadata.Builder().putString(android.media.MediaMetadata.METADATA_KEY_TITLE,title).build())
...
override fun onStop(id:String,i:Boolean){jsEval("window._sdone&&_sdone()")}})   /* network voices can end here, not onDone; page sp-guard keeps manual pause from chaining */
...
w=WebView(this).apply{settings.javaScriptEnabled=true;settings.domStorageEnabled=true;settings.mediaPlaybackRequiresUserGesture=false;addJavascriptInterface(this@M,"A");setBackgroundColor(Color.BLACK)   /* domStorage: localStorage is NULL in WebView without it (music trim cache, page freq counters) */
setRendererPriorityPolicy(WebView.RENDERER_PRIORITY_IMPORTANT,false)   /* keep renderer priority when invisible (frozen renderer killed background music); no-gesture play = shade button works */
...
fun note(c:Context):Notification{
nm(c).createNotificationChannel(NotificationChannel("med","a media",NotificationManager.IMPORTANCE_LOW))
val b=Notification.Builder(c,"med").setSmallIcon(android.R.drawable.ic_media_play).setContentTitle(title.ifEmpty{"a music"}).setOngoing(playing).setContentIntent(PendingIntent.getActivity(c,0,Intent(c,M::class.java),PendingIntent.FLAG_IMMUTABLE))
tok?.let{b.setStyle(Notification.MediaStyle().setMediaSession(it))}
return b.build()}
fun upd(c:Context){nm(c).notify(9,note(c))}}
...
def _st_push(serial,pkg):   # always push after install: no-op on -r, beats Google Backup's stale racing restore; force-stop so nothing caches pre-restore prefs
    ...
    n=0
    for nm,scr in(("prefs","tar -xzf -"),("webls","mkdir -p app_webview/Default && cd app_webview/Default && tar -xzf -")):
...
def run():
    ...
    if proj:
        ...
        try:
            ...
            open(f"{_res}/mipmap-anydpi-v26/ic_launcher.xml","w").write('<adaptive-icon xmlns:android="http://schemas.android.com/apk/res/android"><background android:drawable="@android:color/black"/><foreground android:drawable="@mipmap/ic_fg"/></adaptive-icon>')
            _ico=' android:icon="@mipmap/ic_launcher"'
        except Exception as _e:print("! icon skipped:",_e)
        mf=MF.replace('android:label="a app"','android:label="a app"'+_ico).replace('android:label="a (self)"','android:label="a (self)"'+_ico)
